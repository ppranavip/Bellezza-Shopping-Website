...
def add_products():
    ...
    if request.method == "POST":
        ...
        file = request.files['image']
        if file and allowed_file(file.filename):
            # Preserve spaces in the filename without replacing them
            filename = secure_filename(data['name'][0]) + os.path.splitext(file.filename)[1]
            # Save the file with the preserved filename
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            data['image'] = filename  # Add the image filename to the product data

            # Ensure all elements in the data dictionary are scalar values
